"""Model-guided targeted-panel enrichment — f-only (motif) on the FULL 24-clone cohort.

The Levanon panel claim: pick a small top-K% region by the model's predicted off-target
preference -> pooled editing index FAR above genome-wide. Metric = parent-subtracted DEI
fold at K = 0.1/0.5/1/5/10%, with random-panel and naive-TpC-gate controls.

f-only model: rAPOBEC1 trinucleotide (NCN, edited C centered) log-odds, learned from
data/processed/canonical_sites.parquet (CBE/rAPOBEC1 PUBLISHED off-targets, out-of-sample
vs our WGS) against the candidate-C background. Score every panel C, rank, take top-K%.

This is the clean 24-clone baseline that the f x g (accessibility) arm must beat, and a
stand-in for the eventual Phase-2 model (Evo sequence + structure embeddings per site).
Inputs: poc24_results/per_site_labels_24clone.parquet, /tmp/poc_dna/ref/hg19.fa,
data/processed/canonical_sites.parquet. Writes poc24_results/enrichment_fonly_24clone.txt.
"""
import os, sys, time
import numpy as np, pandas as pd

REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
RES = os.path.join(REPO, 'experiments/phase4_index/poc24_results')
SITES = f'{RES}/per_site_labels_24clone.parquet'
...
# 1. rAPOBEC1 trinucleotide spectrum from published catalog (out-of-sample)
...
# 3. trinucleotide context per position from hg19 (C-centered, edited strand)
log('streaming hg19.fa for trinucleotide context...')
by = {}
for i, (c, p, s) in enumerate(zip(st.chrom.values, st.pos.values, st.strand.values)):
    by.setdefault(c, []).append(i)
trinuc = np.empty(N, dtype=object); want = set(by); cur = None; parts = None
def emit(chrom, seq):
    L = len(seq)
    for i in by[chrom]:
        p = int(st.pos.values[i]); j = p - 1
...
with open(REF) as fh:
    ...
    if cur is not None and parts is not None: emit(cur, ''.join(parts).upper())
st['trinuc'] = trinuc
ok = st.trinuc.str.match('^[ACGT]C[ACGT]$').fillna(False)
log(f'well-formed C-centered trinuc: {ok.sum():,}/{N:,}')

# 4. f-only log-odds
bg_counts = st.loc[ok, 'trinuc'].value_counts()
all_tri = sorted(set(bg_counts.index) | set(off_counts.index)); a = 1.0
off_tot = off_counts.sum() + a * len(all_tri); bg_tot = bg_counts.sum() + a * len(all_tri)
logodds = {t: np.log((off_counts.get(t, 0) + a) / off_tot) - np.log((bg_counts.get(t, 0) + a) / bg_tot)
           for t in all_tri}
st['f'] = st.trinuc.map(logodds).fillna(-1e9).astype(float); st.loc[~ok, 'f'] = -1e9
...
glob = {k: EI(np.ones(N, bool), ed[k], tot[k]) for k in cohorts}; par_glob = glob['Parent']

rep = []
rep.append("=== MODEL-GUIDED TARGETED-PANEL ENRICHMENT — f-only (rAPOBEC1 trinuc), 24 clones ===")
...
rep.append(f"{'K%':>5} {'#sites':>10} {'#genes':>7} {'BE4 EI':>10} {'BE4 fold':>9} {'BE4 DEI':>10} {'DEI fold':>9} {'nCas9 fold':>10} {'Parent fold':>11}")
for K in KS:
    n = max(1, int(N * K / 100)); sel = np.zeros(N, bool); sel[order[:n]] = True
    ...
    dei_top = be4_top - par_top
    be4_fold = be4_top / glob['BE4'] if glob['BE4'] else float('nan')
    dei_fold = dei_top / (glob['BE4'] - par_glob) if (glob['BE4'] - par_glob) else float('nan')
    nc_fold = nc_top / glob['nCas9'] if glob['nCas9'] else float('nan')
    par_fold = par_top / par_glob if par_glob else float('nan')
    rep.append(f"{K:>5} {n:>10,} {ngenes:>7,} {be4_top:>10.3e} {be4_fold:>9.2f} {dei_top:>+10.3e} {dei_fold:>9.2f} {nc_fold:>10.2f} {par_fold:>11.2f}")

rep.append("\n--- size-matched RANDOM panel control (K=1%), BE4 ---")
n1 = max(1, int(N * 0.01)); rfolds = []
for seed in range(5):
...
txt = '\n'.join(rep); print(txt)
open(f'{RES}/enrichment_fonly_24clone.txt', 'w').write(txt + '\n')
st[['chrom', 'pos', 'strand', 'gene', 'motif', 'trinuc', 'f']].to_parquet(f'{RES}/site_fscore_fonly_24clone.parquet')
log('DONE -> enrichment_fonly_24clone.txt + site_fscore_fonly_24clone.parquet')
