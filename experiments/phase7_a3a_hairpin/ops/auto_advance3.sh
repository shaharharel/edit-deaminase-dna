...
#
# v3 ADDS A CALIBRATOR QUALIFICATION GATE (Step A0) ahead of everything else.
# v2 ADDED THE RECURRENCE-FILTER COMPARISON, which has been a recorded pending task since the
...
exec >> $LOG 2>&1
echo "[$(date -u +%FT%H:%M)] v3 armed (Step A0 calibrator gate + recurrence-filter comparison); waiting for P66-D10A-clone6 counts (23/23)"
while [ "$(ls $BASE/feat/counts_P66-D10A-clone6_chr*.npz 2>/dev/null | wc -l)" -lt 23 ]; do
  sleep 300
done
echo "[$(date -u +%FT%H:%M)] clone6 complete. Step A0: is the calibrator technically sound?"
# D10A-clone1 looked like a calibrator and was not: a 7.5x raw noise floor with 87% of its
# alt>=1 sites below VAF 0.05. It silently inverted Endpoint A before anyone looked at the
# VAF profile. Without this gate the driver would compute a full read-out against a defective
# calibrator and the output would look exactly like a result.
if ! $PY $BASE/qualify_calibrator.py P66-D10A-clone6; then
  echo "[$(date -u +%FT%H:%M)] CALIBRATOR DISQUALIFIED. Stopping before any editor number is"
  echo "  produced. Next options: D10A-clone10 (~16:10 UTC) or nCas9 with the cross-study"
  echo "  caveat that the 05:20 preview measured."
  exit 2
fi
echo "[$(date -u +%FT%H:%M)] Step A: cross-study gate."
