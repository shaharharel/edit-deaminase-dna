...
def main():
    ...
    log("using strand-oriented windows from stage2b (no re-read of hg19)")
    ...
    for j, o in enumerate(offs):
        ...
        for b in range(4):
            sq[:, j * 4 + b] = (col == b)
    sq_names = [f"seq{o:+d}_{'ACGT'[b]}" for o in offs for b in range(4)]
    ...
    for bname, (X, names) in blocks.items():
        ...
        np.save(f"{FEAT}/oof_v2_{bname}.npy", oof)
    ...
    log("=" * 64)
    json.dump(results, open(f"{FEAT}/s3_results_v2.json", "w"), indent=2)
    open(f"{BASE}/flags/S3_TRAIN_V2_DONE", "w").write("ok\n")
